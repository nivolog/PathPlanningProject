...
void XmlLogger::writeToLogHPpath(const std::list<Node> &hppath)
{
    ...
    while (iter != --hppath.end()) {
        ...
        part->SetAttribute(CNS_TAG_ATTR_STX, it->y);
        part->SetAttribute(CNS_TAG_ATTR_STY, it->x);
        ++iter;
        part->SetAttribute(CNS_TAG_ATTR_FINX, iter->y);
        part->SetAttribute(CNS_TAG_ATTR_FINY, iter->x);
